...
Parquet file per document, or per sheet when there is more than one, each
with its own columns, guid-partitioned and named for the sheet or the guid)
and the values written into it, since neither needs a real bucket to test.
"""
...
def test_document_gets_its_own_parquet_file_with_typed_bookkeeping():
    ...
    table = pq.read_table(io.BytesIO(client._bucket.blobs["test/guid=abc-123/abc-123.parquet"].data))
...
def test_sheets_get_their_own_file_not_one_wide_unioned_table():
    """Rows tagged _sheet_name (a multi-sheet document) get one file per
    sheet, each with only that sheet's own columns - not one file unioning
    every sheet's columns into a mostly-NULL table. _sheet_name is dropped
    from the row content once it's the filename itself."""
    client = _StubClient()
    rows = [
        {"Week": "51", "Agent": "x", "_valid": True, "_row_number": 2, "_sheet_name": "Andreea Dobrica"},
        {"Objective": "Improvement Required", "Conc": "y", "_valid": True, "_row_number": 2, "_sheet_name": "Reference"},
    ]
    r = json.loads(_writer(client)({"guid": "multi-1", "extracted_rows": rows}))

    assert r["status"] == "success", r
    assert r["documents_written"] == 1, "one document, even though it wrote two files"
    assert r["files_written"] == 2
    assert {f["sheet_name"] for f in r["files"]} == {"Andreea Dobrica", "Reference"}

    assert set(client._bucket.blobs.keys()) == {
        "test/guid=multi-1/Andreea_Dobrica.parquet",
        "test/guid=multi-1/Reference.parquet",
    }

    dobrica = pq.read_table(io.BytesIO(client._bucket.blobs["test/guid=multi-1/Andreea_Dobrica.parquet"].data))
    assert dobrica.column_names == ["Week", "Agent", "_valid", "_row_number"]
    assert "_sheet_name" not in dobrica.column_names
    assert "Objective" not in dobrica.column_names, "must not carry the other sheet's columns"

    reference = pq.read_table(io.BytesIO(client._bucket.blobs["test/guid=multi-1/Reference.parquet"].data))
    assert reference.column_names == ["Objective", "Conc", "_valid", "_row_number"]
...
def test_sheet_name_is_slugified_for_a_safe_filename():
    ...
    client = _StubClient()
    rows = [{"a": "1", "_valid": True, "_row_number": 1, "_sheet_name": "Week 1,,,,Week 2,,"}]
    r = json.loads(_writer(client)({"guid": "messy-1", "extracted_rows": rows}))

    assert r["status"] == "success", r
    [path] = client._bucket.blobs.keys()
    assert path.startswith("test/guid=messy-1/")
    assert path.endswith(".parquet")
    assert "," not in path and " " not in path

    print("✓ test_sheet_name_is_slugified_for_a_safe_filename PASSED")
...
def test_path_is_guid_partitioned_so_reextraction_overwrites():
    ...
    tool = _writer()
    assert tool.blob_path("g1") == "test/guid=g1/g1.parquet"
    assert tool.uri("g1") == "gs://test-bucket/test/guid=g1/g1.parquet"

    client = _StubClient()
    writer = _writer(client)
    writer({"guid": "g1", "extracted_rows": [{"a": "1", "_valid": True, "_row_number": 1}]})
    first = client._bucket.blobs["test/guid=g1/g1.parquet"].data
    writer({"guid": "g1", "extracted_rows": [{"a": "2", "_valid": True, "_row_number": 1}]})
    second = client._bucket.blobs["test/guid=g1/g1.parquet"].data
